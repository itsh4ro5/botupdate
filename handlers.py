...
from pyrogram.errors import (
    SessionPasswordNeeded, FloodWait, RPCError, 
...
async def set_role_based_commands(user_id: int, client: Client):
    try:
        ...
        if str(user_id) == str(OWNER_ID) or is_admin(user_id):
            ...
            await client.set_bot_commands(user_cmds, scope=BotCommandScopeChat(chat_id=user_id))
    except Exception:
        pass
...
async def cmd_userbotphone(client: Client, message: Message):
    ...
    if not API_ID or API_ID == 0:
        return await message.reply_text("❌ **API_ID Missing!** Kripya Cloud Dashboard me API_ID theek karein.", parse_mode=ParseMode.MARKDOWN)
    ...
    try:
        ...
        await msg.edit_text("✅ **OTP Bhej diya gaya hai!**\n\nKripya apna OTP yahan type karein.\n⚠️ **DHYAN DEIN:** OTP space lagakar likhein (Example: `1 2 3 4 5`)", parse_mode=ParseMode.MARKDOWN)
...
async def cmd_userbototp(client: Client, message: Message):
    ...
    try:
        ...
        await msg.edit_text("🎉 **LOGIN SUCCESSFUL!**\n\nBina 2FA ke Session Database me save ho gaya hai.", parse_mode=ParseMode.MARKDOWN)
    except SessionPasswordNeeded:
        ADMIN_WIZARD[uid] = {"step": "call_cmd_userbotpass"}
        await msg.edit_text("🔒 **2-Step Verification Detected!**\n\nIs account me 2FA on hai. Kripya apna **2FA Password** type karein:", parse_mode=ParseMode.MARKDOWN)
    except Exception as e:
        await temp_client.disconnect()
        user_store.pop('login_client', None)
...
async def cmd_userbotpass(client: Client, message: Message):
    ...
    try:
        ...
        await msg.edit_text("🎉 **LOGIN SUCCESSFUL!**\n\n2FA Password verified. Session save ho gaya hai.", parse_mode=ParseMode.MARKDOWN)
    except Exception as e:
        await temp_client.disconnect()
        user_store.pop('login_client', None)
...
async def cmd_emptybatch(client: Client, message: Message):
    ...
    try: cid = int(args[0])
    except ValueError: return await message.reply_text("❌ Error: Valid Batch ID bhejein.")
    
    session_string = DB.get("USERBOT_SESSION")
    if not session_string or not API_ID: 
        return await message.reply_text("❌ **Userbot Not Logged In!** Pehle Owner dashboard se login karein.", parse_mode=ParseMode.MARKDOWN)
        
    msg = await message.reply_text(f"⏳ **Emptying Batch `{cid}`...**\nUserbot start ho raha hai. Isme thoda time lag sakta hai, please wait.", parse_mode=ParseMode.MARKDOWN)
    ...
    try:
        ...
        try:
            ...
            await msg.edit_text("🔄 **Userbot Syncing...**\nChat list ko memory me load kar raha hu taaki ID mil sake. Isme 5-10 seconds lag sakte hain...", parse_mode=ParseMode.MARKDOWN)
            async for dialog in userbot.get_dialogs():
                if dialog.chat.id == cid:
                    break
            await msg.edit_text(f"⏳ **Emptying Batch `{cid}`...**\nSync complete! Ab members remove kar raha hu...", parse_mode=ParseMode.MARKDOWN)
...
async def cmd_del_admin(client: Client, message: Message):
    ...
    try:
        ...
        if target in DB["ADMIN_IDS"]:
            ...
            await save_data_async()
            await message.reply_text(f"🗑 User `{target}` removed.", parse_mode=ParseMode.MARKDOWN)
    except Exception: pass
...
async def cmd_joinall(client: Client, message: Message):
    ...
    session_string = DB.get("USERBOT_SESSION")
    if not session_string or not API_ID: 
        return await message.reply_text("❌ Error: Userbot not logged in. Owner dashboard se login karein.")
    msg = await message.reply_text("⏳ Auto-joining userbot...")
    userbot = Client("temp_bot", api_id=API_ID, api_hash=API_HASH, session_string=session_string, in_memory=True)
    try: 
        await userbot.start()
    ...
    for cid in all_chats:
        try: 
            await client.promote_chat_member(
                chat_id=int(cid), 
                user_id=userbot_id, 
                privileges=pyrogram.types.ChatPrivileges(can_invite_users=True, can_manage_chat=True)
            )
            success += 1
...
async def cmd_lockpaid(client: Client, message: Message):
    ...
    await save_data_async()
    await message.reply_text("Paid Batches **LOCKED 🔐**." if DB["PAID_LOCKED"] else "Paid Batches **UNLOCKED 🔓**.", parse_mode=ParseMode.MARKDOWN)
...
async def cmd_unban(client: Client, message: Message):
    ...
    except (ValueError, TypeError): return await message.reply_text("❌ Error: Kripya ek valid Numeric User ID bhejein.")
...
async def cmd_delcat(client: Client, message: Message):
    ...
    await message.reply_text("🗑 **Delete Category:**", reply_markup=InlineKeyboardMarkup(kb), parse_mode=ParseMode.MARKDOWN)
...
async def cmd_extend_demo(client: Client, message: Message):
    ...
    if uid in DB["USER_DATA"] and bid in DB["USER_DATA"].get(uid, {}).get("demos", {}):
        ...
        DB["USER_DATA"][uid]["demos"][bid] = {
            "expiry": max((d["expiry"] if isinstance(d, dict) else float(d)), time.time()) + (hours * 3600), 
            "warned": False
        }
        await save_data_async()
...
async def cmd_approve_demo(client: Client, message: Message):
    ...
    if not link: return await message.reply_text("❌ Link nahi mila.")
    ...
    if not target_uid or not batch_id: return await message.reply_text("❌ Ye link database me registered nahi hai.")
...
async def cmd_approve_perm(client: Client, message: Message):
    ...
    if not link and args: link = args[0]
    if not link: return
...
async def cmd_addbatch_start(client: Client, message: Message):
    ...
    for i in range(0, len(categories), 2):
        ...
        if i+1 < len(categories): row.append(InlineKeyboardButton(categories[i+1], callback_data=f"wcat_{i+1}"))
        kb.append(row)
    await message.reply_text("🆕 **Add Batch Wizard**\nSelect Category:", reply_markup=InlineKeyboardMarkup(kb), parse_mode=ParseMode.MARKDOWN)
...
async def cmd_user_details(client: Client, message: Message):
    ...
    except Exception: return await message.reply_text("Usage: /user [id]")
...
async def cmd_lockdown(client: Client, message: Message):
    ...
    await save_data_async()
    msg = await message.reply_text("🔓 **Lockdown Lifted!**" if DB["NEW_USERS_ALLOWED"] else "🔒 **Lockdown Enabled!**", parse_mode=ParseMode.MARKDOWN)
    await schedule_delete(client, msg)
...
async def cmd_lockfree(client: Client, message: Message):
    ...
    await save_data_async()
    await message.reply_text("Free Batches **LOCKED 🔒**." if DB["FREE_LOCKED"] else "Free Batches **UNLOCKED 🔓**.", parse_mode=ParseMode.MARKDOWN)

async def cmd_locktestbot(client: Client, message: Message):
    if not is_admin(message.from_user.id): return
    DB["TEST_BOT_LOCKED"] = not DB.get("TEST_BOT_LOCKED", False)
    await save_data_async()
    await message.reply_text("Test Bot **LOCKED 🔒**." if DB["TEST_BOT_LOCKED"] else "Test Bot **UNLOCKED 🔓**.", parse_mode=ParseMode.MARKDOWN)
...
async def cmd_clear(client: Client, message: Message):
    ...
    try:
        await userbot.start()
        await msg.edit_text("🔄 **Userbot Syncing...**\nSaare chats ko memory me load kar raha hu (Peer ID Error bachane ke liye)...", parse_mode=ParseMode.MARKDOWN)
        async for _ in userbot.get_dialogs(): pass
        await msg.edit_text("⏳ **Super Exit /clear Start...**\nSync complete! Ab removing process chalu hai...", parse_mode=ParseMode.MARKDOWN)
...
async def wizard_message(client: Client, message: Message):
    if not message.from_user or message.from_user.id not in ADMIN_WIZARD: return False
    ...
    if state["step"] == "ask_id":
        try:
            ...
            if state["type"] == "free":
                b_count = 0
                await message.reply_text("📢 Sending Auto-Broadcast...", parse_mode=ParseMode.MARKDOWN)
                for t_cid in list(DB["ALL_CHATS"].keys()):
                    if t_cid != cid: 
                        try:
                            sent_msg = await client.send_message(int(t_cid), f"🎉 <b>NEW FREE BATCH ADDED!</b>\n📛 Name: {cname}\n👉 Join via Bot Menu!", parse_mode=ParseMode.HTML)
                            DB.setdefault("SCHEDULED_DELETES", []).append({"c": int(t_cid), "m": sent_msg.id, "t": time.time() + 10800 })
                            b_count += 1
                        except Exception: pass
        ...
        try:
            cmds = {
                "addadmin": cmd_add_admin, "deladmin": cmd_del_admin, "ban": cmd_ban, 
                "unban": cmd_unban, "kick": cmd_kick_user, "find": cmd_find_user, 
                "resetuser": cmd_reset_user, "demo": cmd_approve_demo, "perm": cmd_approve_perm, 
                "extend": cmd_extend_demo, "settestbot": cmd_set_testbot, "setwelcome": cmd_set_welcome, 
                "delbatch": cmd_delbatch, "addcat": cmd_addcat, "setcat": cmd_setcategory, 
                "emptybatch": cmd_emptybatch, "userbotphone": cmd_userbotphone, 
                "userbototp": cmd_userbototp, "userbotpass": cmd_userbotpass
            }
            if cmd_name in cmds: await cmds[cmd_name](client, message)
        except Exception: pass
...
async def handle_broadcast_flow(client: Client, message: Message):
    if not message.from_user or message.from_user.id not in BROADCAST_STATE: return False
    ...
    if state["step"] == "wait_msg":
        ...
        await message.reply_text("📢 **Confirm?**", reply_markup=InlineKeyboardMarkup(kb), parse_mode=ParseMode.MARKDOWN)
        return True
    return False
...
async def general_callback(client: Client, q: CallbackQuery):
    ...
    try:
        if data.startswith("wiz_") or data.startswith("wcat_"): return await wizard_callback(client, q)
        if data.startswith("bc_"): return await broadcast_callback(client, q)
        ...
        if data == "dash_home":
            ...
            kb = [
                ...
                [InlineKeyboardButton("📂 Set Batch Category", callback_data="input_setcat"), InlineKeyboardButton("🧹 Empty Batch", callback_data="input_emptybatch")], 
            ...
            if session:
                ...
                text = "🛡️ **USERBOT CONTROL PANEL**\n\n❌ **Status:** 🔴 NOT LOGGED IN\n\n⚠️ *Koi active session nahi hai. Userbot features won't work. Kripya login karein.*"
            ...
            prompts = {
                "addadmin": "Send User ID to make Admin:", "deladmin": "Send User ID to remove from Admin:", 
                "ban": "Send User ID to Ban:", "unban": "Send User ID to Unban:", 
                "kick": "Send User ID and Batch ID\nFormat: `uid bid`", "find": "Send Username to find:", 
                "resetuser": "Send User ID to reset:", "demo": "Send Link and Time:\nFormat: `link 10h`", 
                "perm": "Send Link to approve:", "extend": "Send User ID, Batch ID, Hours:\nFormat: `uid bid 24`", 
                "settestbot": "Send new Test Bot link:", "setwelcome": "Send Batch ID and Welcome Msg:\nFormat: `bid message`", 
                "delbatch": "Send Type and ID:\nFormat: `free 123` or `paid 123`", "addcat": "Send Name for new Category:", 
        ...
        elif data == "act_backup": await q.answer("Sending..."); await cmd_backup(client, q.message)
        elif data == "act_sync": await q.answer("Sync Started!"); await cmd_sync(client, q.message)
        elif data == "act_allusers": await q.answer("Generating..."); await cmd_all_users(client, q.message)
        elif data == "act_batchstats": await q.answer("Calculating..."); await cmd_batch_stats(client, q.message)
        elif data == "act_addbatch": await q.answer(); await cmd_addbatch_start(client, q.message)
        elif data == "act_delcat": await q.answer(); await cmd_delcat(client, q.message)
        elif data == "act_broadcast": await q.answer(); await cmd_broadcast_start(client, q.message)
        elif data == "act_post": await q.answer(); await cmd_post_start(client, q.message)
            ...
            if not await check_membership_pyro(uid, client): return await q.answer("❌ Join Main Channel First!", show_alert=True)
            ...
            try: 
                sent_msg = await client.send_message(uid, "🤖 **Test Bot Access Verification:**\n\nYou are verified! Click the button below to open the Test Bot.", reply_markup=InlineKeyboardMarkup(kb), parse_mode=ParseMode.MARKDOWN)
                await schedule_delete(client, sent_msg, delay=60)
            except Exception: pass

        elif data.startswith("my_batches_"):
            await q.answer()
            if not DB["USER_DATA"].get(uid, {}).get("tnc_accepted", False): return await show_tnc_menu_cb(client, q)
            ...
            if not joined_batches: 
                return await q.edit_message_text("❌ Aap abhi kisi bhi batch me join nahi hain.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back to Menu", callback_data="u_main")]]), parse_mode=ParseMode.MARKDOWN)
            ...
            kb = [[InlineKeyboardButton("🆓 Free Batches", callback_data=f"listcat_{cat_idx}_free_0"), InlineKeyboardButton("💎 Paid Batches", callback_data=f"listcat_{cat_idx}_paid_0")], [InlineKeyboardButton("🔙 Back to Categories", callback_data="all_batches_0")]]
            ...
            if not filtered_batches: 
                return await q.edit_message_text(f"❌ Is category ({cat_name}) me abhi koi {b_type.title()} batch nahi hai.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data=f"showcat_{cat_idx}")]]), parse_mode=ParseMode.MARKDOWN)
            MAX_PER_PAGE = 10
            ...
            if await is_already_in_channel_pyro(client, cid, uid): return await q.answer("⚠️ Already Joined!", show_alert=True)
            try:
                bname = DB["ALL_CHATS"].get(cid, f"Batch {cid}")
                l = await client.create_chat_invite_link(cid, creates_join_request=True, name=f"Free-{uid}", expire_date=int(time.time())+60)
                sent_msg = await client.send_message(uid, f"🔗 <b>Link:</b>\n\n<b>{bname}</b>\n\n{l.invite_link}\n\nℹ️ <i>Request auto-approved.</i>\n⏳ <i>(Expires in 1 min)</i>", parse_mode=ParseMode.HTML)
                await schedule_delete(client, sent_msg, delay=60)
            ...
            cid = int(data.split("_")[2])
            if not await check_membership_pyro(uid, client): return await q.answer("❌ Join Main First!", show_alert=True)
            if await is_already_in_channel_pyro(client, cid, uid): return await q.answer("⚠️ Already joined!", show_alert=True)
            await q.answer("🔄 Generating Link...")
            try:
                bname = DB["ALL_CHATS"].get(cid, f"Batch {cid}")
                l = await client.create_chat_invite_link(cid, creates_join_request=True, name=f"Req-{uid}", expire_date=int(time.time())+60)
                DB["LINK_MAP"][l.invite_link] = {"u": uid, "b": cid}
                await save_data_async()
                topic_id = await get_or_create_topic(q.from_user, client)
                if topic_id:
                    try: await client.send_message(SUPPORT_GROUP_ID, f"🔔 **NEW REQUEST**\n👤 User: {q.from_user.mention}\n📂 Batch: <b>{bname}</b>\n🔗 Link: {l.invite_link}\n\n👇 **Action:**\n/demo {l.invite_link}\n/per {l.invite_link}", message_thread_id=topic_id, parse_mode=ParseMode.HTML)
                    except Exception: pass
                user_msg_obj = await client.send_message(uid, f"✅ <b>Access Link Generated!</b>\n\n<b>{bname}</b>\n\n🔗 {l.invite_link}\n\nℹ️ <b>Sent to Admin.</b> Wait for approval.\n⏳ <i>(Expires in 1 min)</i>", parse_mode=ParseMode.HTML)
...
async def start(client: Client, message: Message):
    ...
    if str(user.id) == str(OWNER_ID):
        kb = [
            [InlineKeyboardButton("🔒 Security", callback_data="dash_locks"), InlineKeyboardButton("🗄️ Database", callback_data="dash_db")], 
            [InlineKeyboardButton("📦 Batches", callback_data="dash_batches"), InlineKeyboardButton("👥 Staff", callback_data="dash_staff")], 
            [InlineKeyboardButton("📢 Comms", callback_data="dash_comms"), InlineKeyboardButton("📊 Analytics", callback_data="dash_stats")],
        ...
        await message.reply_text("⚠️ **Join Main Channel First**", reply_markup=InlineKeyboardMarkup(kb), parse_mode=ParseMode.MARKDOWN)
...
async def start_from_cb(client: Client, q: CallbackQuery):
    ...
    if str(user.id) == str(OWNER_ID):
        ...
        kb = [[InlineKeyboardButton("📢 Join Channel", url=MANDATORY_CHANNEL_LINK)], [InlineKeyboardButton("✅ Verified", callback_data="verify")]]
        await q.edit_message_text("⚠️ **Join Main Channel First**", reply_markup=InlineKeyboardMarkup(kb), parse_mode=ParseMode.MARKDOWN)
    ...
    if key in MESSAGE_MAP:
        ...
        try:
            if not m.photo and not m.video and not m.document and not m.audio and not m.animation:
                if m.text: await client.edit_message_text(tc, tm, m.text, entities=m.entities)
            else:
                new_media = None
                if m.photo: new_media = InputMediaPhoto(m.photo.file_id, caption=m.caption, caption_entities=m.caption_entities)
                ...
                if new_media: await client.edit_message_media(tc, tm, media=new_media)
                elif m.caption is not None: await client.edit_message_caption(tc, tm, caption=m.caption, caption_entities=m.caption_entities)
...
async def main_message_handler(client: Client, message: Message):
    ...
    if chat.type == ChatType.PRIVATE:
        ...
        if topic_id:
            ...
            try:
                sent = await message.copy(int(SUPPORT_GROUP_ID), message_thread_id=topic_id, reply_to_message_id=reply_id)
        ...
        if message.from_user.id == (await client.get_me()).id: return
...
async def on_chat_member_update(client: Client, update: ChatMemberUpdated):
    ...
    if str(update.chat.id).replace("-100", "") == str(MANDATORY_CHANNEL_ID).replace("-100", "") and status in [ChatMemberStatus.LEFT, ChatMemberStatus.BANNED]:
    ...
    try:
        ...
        for idx, uid in enumerate(user_ids):
            ...
            try: 
                ...
                elif status == ChatMemberStatus.LEFT and DB["USER_DATA"].get(uid, {}).get("tnc_accepted", False):
                    await execute_universal_kick(user_id, client)
            except Exception: 
                pass
...
async def handle_join_request(client: Client, req: ChatJoinRequest):
    ...
    if user.id in DB["BLOCKED_USERS"]:
        try: await client.decline_chat_join_request(chat.id, user.id)
        except Exception: pass
        return
    if chat.id in DB["FREE_CHANNELS"]:
        if await check_membership_pyro(user.id, client):
            try:
                ...
                w_msg = await client.send_message(user.id, welcome_str, parse_mode=ParseMode.MARKDOWN)
                await schedule_delete(client, w_msg, delay=60)
            except Exception: pass
        else:
            try: 
                await client.send_message(user.id, f"⚠️ **Declined!**\nJoin Main:\n{MANDATORY_CHANNEL_LINK}", parse_mode=ParseMode.MARKDOWN)
        ...
        if req.invite_link and req.invite_link.invite_link in DB["LINK_MAP"]:
            try: await client.revoke_chat_invite_link(chat.id, req.invite_link.invite_link)
            except Exception: pass
...
async def auto_backup_db(client: Client):
    if LOG_CHANNEL_ID and os.path.exists(DATA_FILE):
        try: await client.send_document(int(LOG_CHANNEL_ID), document=DATA_FILE, caption="🛡️ Auto Backup")
        except Exception: pass
