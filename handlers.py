...
async def cmd_del_msg(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if key in MESSAGE_MAP:
        ...
        try:
            await context.bot.delete_message(tc, tm); await update.message.reply_to_message.delete(); await update.message.delete()
            del MESSAGE_MAP[key]
...
async def cmd_sync(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.job_queue.run_once(background_sync, 1); await schedule_delete(context, update.message); await schedule_delete(context, msg)
...
async def cmd_joinall(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    all_chats = [MANDATORY_CHANNEL_ID] + list(DB["FREE_CHANNELS"].keys()) + list(DB["PAID_CHANNELS"].keys()); success = failed = 0
...
async def cmd_all_users(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    f = io.BytesIO(report.encode("utf-8")); f.name = "all_users.txt"; await update.message.reply_document(document=f, caption="✅ All Users List")
...
async def cmd_ban(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if target not in DB["BLOCKED_USERS"] and target != OWNER_ID: 
        await execute_universal_kick(target, context, permanent_ban=True); await update.message.reply_text(f"🚫 User `{target}` BANNED.", parse_mode=ParseMode.MARKDOWN)
...
async def cmd_addcat(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id) or not context.args: return
    new_cat = " ".join(context.args).strip(); categories = DB.get("CATEGORIES", DEFAULT_CATEGORIES)
...
async def cmd_user_details(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    f = io.BytesIO(r.encode("utf-8")); f.name = f"scan_{target_id}.txt"; await update.message.reply_document(document=f, caption="🔍 Deep Scan")
...
async def cmd_batches(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    f = io.BytesIO(r.encode("utf-8")); f.name = "batches.txt"; await update.message.reply_document(document=f)
