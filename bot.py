...
# 1. FLASK WEB SERVER (Keeps Hugging Face Space 'Running 🟢' instantly!)
app = Flask(__name__)
...
@app.route("/health")
def health():
    return jsonify({"status": "healthy", "engine": "Pyrogram MTProto Hybrid"})


def run_flask():
    port = int(os.environ.get("PORT", 7860))
    print(f"🟢 BOOT[2/4]: Binding Flask Web Dashboard on port {port}...", flush=True)
    # Flask starts synchronously, satisfying HF's health check within milliseconds!
    app.run(host="0.0.0.0", port=port, use_reloader=False)


# 2. PYROGRAM BOT BACKGROUND WORKER
def run_bot_thread():
    print("🟢 BOOT[3/4]: Initializing Bot Engine in Background Thread...", flush=True)
    ...
    try:
        ...
        bot = Client(
            ...
            in_memory=True,  # Saves Disk I/O on cloud containers
            workers=50,      # 50 Parallel async workers for turbo speed!
        )
        config.bot_app = bot
            ...
            if OWNER_ID:
                loop.run_until_complete(
                    bot.send_message(
                        ...
                        "⚡ **Status:** `Running Smoothly (Flask + Pyrogram Hybrid)`\n"
        ...
        # Background Tasks Scheduler Loop
        while True:
            time.sleep(60)
            try:
                if hasattr(H, "check_demos"):
                    loop.run_until_complete(H.check_demos(bot))
            except Exception as e:
        ...
        traceback.print_exc()
        print("=========================================================", flush=True)
        print("💡 Note: Flask Server abhi bhi chalu hai taaki Space 'Running' rahe aur tum logs padh sako!", flush=True)
        while True:
            time.sleep(60)  # Keeps thread alive so Docker never restarts the Space


if __name__ == "__main__":
    # Step 1: Launch Pyrogram Bot in a separate Background Thread
    bot_worker = threading.Thread(target=run_bot_thread, daemon=True)
    bot_worker.start()

    # Step 2: Launch Flask Web Server on Main Thread (Instantly opens Port 7860)
    run_flask()
